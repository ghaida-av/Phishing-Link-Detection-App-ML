...
class Database:
    ...
    def init_database(self):
        ...
        db_pointer.execute('CREATE INDEX IF NOT EXISTS idx_created_at ON urls(created)')
    ...
    def save_url_result(self, url: str, url_type: str,
                        is_phishing: bool, confidence: float) -> bool:
        conn = self.get_connection()
        db_pointer = conn.cursor()
        ...
        try:
            db_pointer.execute('''
                ...
                (url, urltype, is_phishing, confidence, created)
                VALUES (?, ?, ?, ?, ?)
            ''', (url, urltype, int(is_phishing), confidence, datetime.now()))
